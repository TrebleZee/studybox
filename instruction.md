...
- A paper's date is the user's `examDate` if set, otherwise the file's date for `<board>-<spec>` (lowercase) and the paper id (`paperExamDate`). User dates always win, and clearing one falls back to the published date. Subjects without papers or off the catalogue only have dates the user adds.
